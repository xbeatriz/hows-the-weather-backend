...
export async function processUserNotifications(user) {
  //Buscar sensores da localização do user
  const sensors = await Sensor.find({ location: user.location });
  if (!sensors.length) return;

  // Buscar últimas leituras dos sensores (exemplo genérico)
  const sensorReadings = sensors.map((sensor) => sensor.last_reading); // Ajustar conforme modelo

  // Para cada config do user, verificar alertas e enviar notificações
  for (const config of user.configs || []) {
    ...
    filteredSensors.forEach((sensor) => {
      ...
      if (config.alert_thresholds.temperature) {
        if (
          t < config.alert_thresholds.temperature.min ||
          t > config.alert_thresholds.temperature.max
        ) {
          alertTriggered = true;
      ...
      if (config.alert_thresholds.humidity) {
        if (
          h < config.alert_thresholds.humidity.min ||
          h > config.alert_thresholds.humidity.max
        ) {
          alertTriggered = true;
      ...
      if (config.alert_thresholds.gas) {
        if (
          g < config.alert_thresholds.gas.min ||
          g > config.alert_thresholds.gas.max
        ) {
          alertTriggered = true;
